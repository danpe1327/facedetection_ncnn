...
VanFace::VanFace(const string& model_path)
{
    ...
    string param_files = model_path + "/VanFace.param";
    string bin_files = model_path + "/VanFace.bin";
...
void VanFace::getSenseBbox(vector<Bbox>& faceBbox, int imgHeight, int imgWidth, vector<Rect>& senseBbox)
{
    ...
    for (int i = 0; i < num_box; i++)
    {
        ...
        faceX1 = (std::min)(faceBbox[i].ppoint[0], faceBbox[i].ppoint[3]) - 0.2 * faceW;
        faceY1 = (std::min)(faceBbox[i].ppoint[5], faceBbox[i].ppoint[6]) - 0.1 * faceH;
        faceX2 = (std::max)(faceBbox[i].ppoint[1], faceBbox[i].ppoint[4]) + 0.2 * faceW;
        faceY2 = (std::max)(faceBbox[i].ppoint[8], faceBbox[i].ppoint[8]) + 0.2 * faceH;
...
void VanFace::detectLandmark(Mat& img_, vector<Bbox>& faceBbox, vector<vector<Point>>& outLandmarks)
{
    img = img_;
    ...
    for (vector<Rect>::iterator it = senseBbox.begin(); it != senseBbox.end(); it++)
    {
        ...
        vector<Point> faceLandmark;
        for (int i = 0; i < keyPoint.total() / 2; i++)
        {
            Point x = Point(int(keyPoint[2 * i] * (it->width) + it->x), int(keyPoint[2 * i + 1] * (it->height) + it->y));
            faceLandmark.push_back(x);
        }

        outLandmarks.push_back(faceLandmark);
        free(data);
    }
}
